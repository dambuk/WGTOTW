...
class User extends \Anax\MVC\CDatabaseModel
{
  ...
  public function isLoggedIn() {
   ...
   // $this->db->setVerbose();
    ...
    public function findMostquestions() {
    
    $user = $this->db->select('t0.*, t1.acronym as name1, COUNT(t0.acronym) AS total')
             ->from($this->getSource(). ' AS t0')
             ->join('questions AS t1', 't0.acronym = t1.acronym')
             ->groupBy('t0.acronym')
             ->orderBy('total DESC')
             ->limit(3)
             ->executeFetchAll();
    ...
    return $user;
      ...
      public function findNumquestions($acronym) {
...
left join 
	(select wgtotw_questions.acronym, count(wgtotw_questions.acronym) AS count2 
     from wgtotw_questions group by wgtotw_questions.acronym)
